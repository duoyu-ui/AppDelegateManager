            ...
            [_singetonInstance.manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
                switch (status) {
                    case AFNetworkReachabilityStatusUnknown: {
                        // 打印日志
                        FYLog(NSLocalizedString(@"网络状态监听 => 未识别的网络", nil));
                    ...
                    case AFNetworkReachabilityStatusNotReachable: {
                        // 打印日志
                        FYLog(NSLocalizedString(@"网络状态监听 => 不可达的网络(未连接)", nil));
                    ...
                    case AFNetworkReachabilityStatusReachableViaWWAN: {
                        // 打印日志
                        FYLog(NSLocalizedString(@"网络状态监听 => 2G,3G,4G...的网络", nil));
                    ...
                    case AFNetworkReachabilityStatusReachableViaWiFi: {
                        // 打印日志
                        FYLog(NSLocalizedString(@"网络状态监听 => WIFI的网络", nil));
...
+ (CFCNetworkStatus)currentNetworktatus
{
    ...
    reach.reachableBlock = ^(Reachability* reach) {
        ...
        switch (netStatus) {
            case NotReachable: {
                FYLog(NSLocalizedString(@"网络状态监听 => 不可达的网络(未连接)", nil));
                [CFCNetworkReachabilityUtil sharedNetworkUtilsWithCurrentReachabilityStatusBlock:nil].networktatus = CFCNetworkStatusNotReachable;
                break;
            }
            case ReachableViaWiFi:
                FYLog(NSLocalizedString(@"网络状态监听 => WIFI的网络", nil));
                [CFCNetworkReachabilityUtil sharedNetworkUtilsWithCurrentReachabilityStatusBlock:nil].networktatus = CFCNetworkStatusReachableViaWiFi;
                break;
            case ReachableViaWWAN:
                FYLog(NSLocalizedString(@"网络状态监听 => 2G,3G,4G...的网络", nil));
                [CFCNetworkReachabilityUtil sharedNetworkUtilsWithCurrentReachabilityStatusBlock:nil].networktatus = CFCNetworkStatusReachableViaWWAN;
                break;
        }
    };
