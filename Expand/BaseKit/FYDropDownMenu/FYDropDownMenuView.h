
#import <UIKit/UIKit.h>
#import "FYDropDownMenuBasedModel.h"
#import "FYDropDownMenuBasedCell.h"

@class FYDropDownMenuView;

#define FYDefaultFloat -10.0
#define FYCOLOR(r, g, b, a) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:(a)]

/**
 *  菜单展现的动画类型
 */
typedef NS_ENUM(NSInteger, FYDropDownMenuViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    FYDropDownMenuViewAnimateType_ScaleBasedTopRight = 0,
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    FYDropDownMenuViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    FYDropDownMenuViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    FYDropDownMenuViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    FYDropDownMenuViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    FYDropDownMenuViewAnimateType_FallFromTop,
};


@protocol FYDropDownMenuViewDelegate <NSObject>
@optional

/** 若是自定义cell样式的，可以在这个代理方法中稍微小修改cell的样式，比如是否需要下划线、字体的颜色等等*/
- (void)fyDropDownMenuView:(FYDropDownMenuView *)menuView WillAppearMenuCell:(FYDropDownMenuBasedCell *)menuCell index:(NSInteger)index;

- (void)fyDropDownMenuViewWillAppear:(FYDropDownMenuView *)menuView;

- (void)fyDropDownMenuViewWDidAppear:(FYDropDownMenuView *)menuView;

- (void)fyDropDownMenuViewWillDisappear:(FYDropDownMenuView *)menuView;

- (void)fyDropDownMenuViewWDidDisappear:(FYDropDownMenuView *)menuView;

@end


/**
 *  下拉菜单
 */
@interface FYDropDownMenuView : UIView

///=========================
///    创建下拉菜单方式1
/// 适用于使用默认的下拉菜单样式
///=========================

/**
 *  快速实例化一个默认样式的下拉菜单
 *  >>>>注意:若还需要对个别属性进行赋值、可以再单独对个别属性进行赋值。但是最后一定要调用 setup方法
 *  @param menuSuperView 菜单父容器
 *  @param menuModelsArray 下拉菜单模型数组,数组中存放着(FFDropDownMenuModel模型对象)
 *  @param menuWidth   菜单的宽度                ---- 若要使用默认宽度、传FYDefaultFloat
 *  @param eachItemHeight 每一个选项的高度           ---- 若要使用默认高度、传FYDefaultFloat
 *  @return 实例化的一个下拉菜单对象
 */
+ (instancetype)fyDropDownMenuWithSuperView:(UIView *)menuSuperView menuModelsArray:(NSArray *)menuModelsArray menuWidth:(CGFloat)menuWidth eachItemHeight:(CGFloat)eachItemHeight;


///==================================================================
///                    创建下拉菜单方式2
///   适用于自定义下拉菜单样式，单个属性进行赋值
///   >>步骤:
///   1、 [FYDropDownMenuView alloc] init]
///   2、 对需要赋值的属性进行赋值
///       >>>>>若要使用默认值，可以不对属性进行赋值，或者赋值 FFDefault....
///   3、 调用 setup方法
///==================================================================

/// 1、下拉菜单模型数组(数组里面存放 FYDropDownMenuBasedModel的子类的对象)
@property (nonatomic, strong) NSArray *menuModelsArray;

/** 2、cell的类名, 必须是FYDropDownMenuBasedCell的子类
 *               若用框架自带的cell,直接传@"FYDropDownMenuBasedCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传NSLocalizedString(@"类名.xib", nil)
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"FYDropDownMenuBasedCell"
 */
@property (nonatomic, copy) NSString *cellClassName;
@property (nonatomic, strong) NSArray<NSString *> *cellClassNames;

/// 3、菜单的宽度(若不设置，默认为 150.0)
@property (nonatomic, assign) CGFloat menuWidth;

/// 4、菜单的圆角半径(若不设置，默认为5.0)
@property (nonatomic, assign) CGFloat menuCornerRadius;

/// 5、每一个选项的高度(若不设置，默认为40.0)
@property (nonatomic, assign) CGFloat eachMenuItemHeight;

/// 6、菜单条离屏幕右边的间距(若不设置，默认为10.0)
@property (nonatomic, assign) CGFloat menuRightMargin;

/** 7、菜单选项的背景颜色(若不设置，默认为白色、可以用FYCOLOR(r, g, b, a) 设置带透明度的颜色)
 *  若是自定义cell,如果在自定义cell中设置了cell中子控件的颜色，这个属性将不起作用,您可以在自定义cell中自己设置颜色
 */
@property (nonatomic, strong) UIColor *menuItemBackgroundColor;

/// 8、三角形颜色(若不设置，默认为白色、可以用FYCOLOR(r, g, b, a) 设置带透明度的颜色)
@property (nonatomic, strong) UIColor *triangleColor;

/** 9、三角形相对于keyWindow的y值,也就是相对于屏幕顶部的y值(若不设置，默认为64.0)
 *          注意:整个下拉菜单距离屏幕顶部的距离是由triangleY来控制的
 */
@property (nonatomic, assign) CGFloat triangleY;

/// 10、三角形距离屏幕右边的间距(若不设置，默认为20.0)
@property (nonatomic, assign) CGFloat triangleRightMargin;

/** 11、三角形的size  size.width:代表三角形底部边长，size.height:代表三角形的高度
 *      若不设置，默认为CGSizeMake(18, 10)
 *      若不需要显示三角形，设置方式如下三种,具体使用哪种，取决于你的需求
 *         <1>   CGSizeMake(0, 0)
 *         <2>   CGSizeMake(宽度, 0)
 *         <3>   CGSizeMake(0, 高度)
 */
@property (nonatomic, assign) CGSize triangleSize;

/** 12、背景颜色开始时的透明度(还没展示menu的透明度)(若不设置，默认为0.02)
 *      背景颜色的透明度就是除去菜单外的灰色蒙板的透明度
 */
@property (nonatomic, assign) CGFloat bgColorbeginAlpha;

/** 13、背景颜色结束的的透明度(menu完全展示的透明度)(若不设置，默认为0.2)
 *     背景颜色的透明度就是除去菜单外的灰色蒙板的透明度
 */
@property (nonatomic, assign) CGFloat bgColorEndAlpha;

/// 14、动画效果时间(若不设置，默认为0.2)
@property (nonatomic, assign) CGFloat animateDuration;

/// 15、菜单的弹出的动画效果类型(若不设置，默认为FYDropDownMenuViewAnimateType_ScaleBasedTopRight)
@property (nonatomic, assign) FYDropDownMenuViewAnimateType menuAnimateType;

/// 16、菜单是否需要滚动（若不设置，默认为不可滚动）
@property (nonatomic, assign) BOOL ifShouldScroll;

/// 17、菜单选项栏的高度（若不设置，默认显示全部菜单item的总高度）
@property (nonatomic, assign) CGFloat menuBarHeight;

/// 18、delegate
@property (nonatomic, weak) id<FYDropDownMenuViewDelegate> delegate;

/// 19、菜单视图Y坐标，默认0
@property (nonatomic, assign) CGFloat menuFrameY;


/** 初始化(当所有属性调用完毕，一定要调用这个方法) */
- (void)setup;

/** 弹出菜单 */
- (void)showMenu;

/** 是否已显示 */
- (BOOL)isShowMenu;

/** 重载菜单 */
- (void)reloadMenu;

/** 关闭菜单 */
- (void)dismissMenu:(BOOL)animation;
- (void)dismissMenu:(BOOL)animation then:(void (^)(void))then;

@end

