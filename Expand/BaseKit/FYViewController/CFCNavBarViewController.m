...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 隐藏导航栏底部横线
    if ([self prefersNavigationBarHairlineHeight] <= 0.0f) {
...
{
    return [UIImage imageNamed:ICON_NAVIGATION_BACKGROUND];
}
...
- (NSString *)prefersNavigationBarLeftButtonItemImageNormal
{
    ...
    switch (barButtonItemType) {
        ...
        case CFCNavBarButtonItemTypeDefault: {
            return ICON_NAVIGATION_BAR_BUTTON_BLACK_ARROW;
            break;
        }
            // 自定义
...
- (UIView *)createNavigationBarLeftButtonItem
{
            ...
            ctl_btn = [self createNavigationBarButtonItemTypeCustomTitle:title
                                                        ...
                                                        titleSelectColor:titleSelectColor
                                                          ...
                                                          iconNameSelect:iconNameSelect
                                                                  action:actionSel
                                                                  target:self
                                                                iconSize:NAVIGATION_BAR_BUTTON_IMAGE_SIZE];
        }
        default: {
            break;
        }
    ...
    // 控件对齐方式
    if ([ctl_btn isKindOfClass:[UIButton class]]) {
        if (![CFCSysUtil validateStringEmpty:title]) {
            [(UIButton *)ctl_btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
        } else if (![CFCSysUtil validateStringEmpty:iconNameNormal]) {
            [(UIButton *)ctl_btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
        }
    }
...
- (UIView *)createNavigationBarRightButtonItem
{
    ...
    if ([ctl_btn isKindOfClass:[UIButton class]]) {
        if (![CFCSysUtil validateStringEmpty:title]) {
            [(UIButton *)ctl_btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentRight];
        } else if (![CFCSysUtil validateStringEmpty:iconNameNormal]) {
            [(UIButton *)ctl_btn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentRight];
        }
    }
                                                   ...
                                                   target:(id)target
{
    ...
    // 默认显示图片
    if (iconNameNormal) {
        CGFloat imageSizeWidth = CFC_AUTOSIZING_WIDTH(20.0f);
//        CGFloat imageSizeHeight = CFC_AUTOSIZING_HEIGTH(20.0f);
        ...
        [btn setImage:[[UIImage imageNamed:iconNameNormal] imageByScalingProportionallyToSize:CGSizeMake(imageSizeWidth, imageSizeWidth)]
             forState:UIControlStateNormal];
        [btn setImage:[[UIImage imageNamed:iconNameSelect] imageByScalingProportionallyToSize:CGSizeMake(imageSizeWidth, imageSizeWidth)]
                                                ...
                                                iconSize:(CGFloat)iconSize
{
    ...
    // 自定义按钮
    CGFloat imageSizeMax = iconSize > NAVIGATION_BAR_BUTTON_IMAGE_SIZE ? NAVIGATION_BAR_BUTTON_IMAGE_SIZE : iconSize;
    CGFloat imageSize = CFC_AUTOSIZING_WIDTH(imageSizeMax);
    CGFloat maxWidth = MAX(MARGIN*1.5+titleWidth, NAVIGATION_BAR_BUTTON_MAX_WIDTH);
...
#pragma mark - UIBarButtonItem UIBarButton
...
#pragma mark 创建导航 UIBarButtonItem 文字
- (UIBarButtonItem *)createBarButtonItemWithTitle:(NSString *)title action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createBarButtonItemWithTitle:title target:self action:action offsetType:offsetType];
}

#pragma mark 创建导航 UIBarButtonItem 文字
- (UIBarButtonItem *)createBarButtonItemWithTitle:(NSString *)title target:(id)target action:(SEL)action
{
    return [self createBarButtonItemWithTitle:title target:target action:action offsetType:CFCNavBarButtonOffsetTypeNone];
}

#pragma mark 创建导航 UIBarButtonItem 文字
- (UIBarButtonItem *)createBarButtonItemWithTitle:(NSString *)title target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    ...
    CGFloat titleWidth = [title widthWithFont:titleFont constrainedToHeight:NAVIGATION_BAR_HEIGHT];
    
    if (CFCNavBarButtonOffsetTypeLeft == offsetType) {
        offset = titleWidth - NAVIGATION_BAR_BUTTON_MAX_WIDTH;
        edgeInsets = UIEdgeInsetsMake(0.0f, offset, 0.0f, 0.0f);
    } else if (CFCNavBarButtonOffsetTypeRight == offsetType) {
        offset = titleWidth - NAVIGATION_BAR_BUTTON_MAX_WIDTH;
        edgeInsets = UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, offset);
    }
...
- (UIBarButtonItem *)createBarButtonItemWithImage:(NSString *)image action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createBarButtonItemWithImage:image target:self action:action offsetType:offsetType];
}

#pragma mark 创建导航 UIBarButtonItem 图片
- (UIBarButtonItem *)createBarButtonItemWithImage:(NSString *)image target:(id)target action:(SEL)action
{
    return [self createBarButtonItemWithImage:image target:target action:action offsetType:CFCNavBarButtonOffsetTypeNone];
}

#pragma mark 创建导航 UIBarButtonItem 图片
- (UIBarButtonItem *)createBarButtonItemWithImage:(NSString *)image target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createBarButtonItemWithImage:image target:target action:action offsetType:offsetType imageSize:NAVIGATION_BAR_BUTTON_IMAGE_SIZE];
}

#pragma mark 创建导航 UIBarButtonItem 图片
- (UIBarButtonItem *)createBarButtonItemWithImage:(NSString *)image target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType imageSize:(CGFloat)imageSize
{
    CGFloat offset = 0.0f;
    if (CFCNavBarButtonOffsetTypeLeft == offsetType) {
        offset = NAVIGATION_BAR_BUTTON_MAX_WIDTH - CFC_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_IMAGE_SIZE);
    } else if (CFCNavBarButtonOffsetTypeRight == offsetType) {
        offset = CFC_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_IMAGE_SIZE) - NAVIGATION_BAR_BUTTON_MAX_WIDTH;
    }
    return [self createBarButtonItemWithImage:image
                             highLightedImage:nil
                              imageEdgeInsets:UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, offset)
                                       target:target
                                       action:action
                                    imageSize:imageSize];
}
...
{
    UIButton *button = [self createButtonWithImage:image highLightedImage:highLightedImage imageEdgeInsets:imageEdgeInsets target:target action:action imageSize:imageSize];
    return [[UIBarButtonItem alloc] initWithCustomView:button];
}
...
#pragma mark 创建导航 UIButton 文字
- (UIButton *)createButtonWithTitle:(NSString *)title action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createButtonWithTitle:title target:self action:action offsetType:offsetType];
}

#pragma mark 创建导航 UIButton 文字
- (UIButton *)createButtonWithTitle:(NSString *)title target:(id)target action:(SEL)action
{
    return [self createButtonWithTitle:title target:target action:action offsetType:CFCNavBarButtonOffsetTypeNone];
}

#pragma mark 创建导航 UIButton 文字
- (UIButton *)createButtonWithTitle:(NSString *)title target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    UIFont *titleFont = [self prefersNavigationBarButtonItemTitleFont];
    UIColor *titleNormalColor = [self prefersNavigationBarButtonItemTitleColorNormal];
    UIColor *titleSelectColor = [self prefersNavigationBarButtonItemTitleColorSelect];
    ...
    if (CFCNavBarButtonOffsetTypeLeft == offsetType) {
        offset = NAVIGATION_BAR_BUTTON_MAX_WIDTH - titleWidth;
        edgeInsets = UIEdgeInsetsMake(0.0f, offset, 0.0f, 0.0f);
    } else if (CFCNavBarButtonOffsetTypeRight == offsetType) {
        offset = titleWidth - NAVIGATION_BAR_BUTTON_MAX_WIDTH;
        edgeInsets = UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, offset);
...
#pragma mark 创建导航 UIButton 图片
- (UIButton *)createButtonWithImage:(NSString *)image action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createButtonWithImage:image target:self action:action offsetType:offsetType];
}

#pragma mark 创建导航 UIButton 图片
- (UIButton *)createButtonWithImage:(NSString *)image target:(id)target action:(SEL)action
{
    return [self createButtonWithImage:image target:target action:action offsetType:CFCNavBarButtonOffsetTypeNone];
}

#pragma mark 创建导航 UIButton 图片
- (UIButton *)createButtonWithImage:(NSString *)image target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType
{
    return [self createButtonWithImage:image target:target action:action offsetType:offsetType imageSize:NAVIGATION_BAR_BUTTON_IMAGE_SIZE];
}

#pragma mark 创建导航 UIButton 图片
- (UIButton *)createButtonWithImage:(NSString *)image target:(id)target action:(SEL)action offsetType:(CFCNavBarButtonOffsetType)offsetType imageSize:(CGFloat)imageSize
{
    CGFloat offset = 0.0f;
    if (CFCNavBarButtonOffsetTypeLeft == offsetType) {
        offset = NAVIGATION_BAR_BUTTON_MAX_WIDTH - CFC_AUTOSIZING_WIDTH(NAVIGATION_BAR_BUTTON_IMAGE_SIZE);
    ...
    return [self createButtonWithImage:image
                      highLightedImage:nil
                       imageEdgeInsets:UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, offset)
                                target:target
                                action:action
...
- (void)setupNavigationBarViewCustom
{
    ...
    if ([self prefersNavigationBarLeftButtonItemType]) {
        ...
        self.navigationBarLeftButtonItem = [self createNavigationBarLeftButtonItem];
        if (self.navigationBarLeftButtonItem) {
            CGFloat nav_margin = CFC_AUTOSIZING_WIDTH(NAVIGATION_BAR_SCREEN_MARGIN);
            // 调整控件位置
            CGRect frame = CGRectMake(self.navigationBarLeftButtonItem.frame.origin.x + nav_margin,
    ...
    if ([self prefersNavigationBarRightButtonItemType]) {
        ...
        if (self.navigationBarRightButtonItem) {
            CGFloat nav_margin = CFC_AUTOSIZING_WIDTH(NAVIGATION_BAR_SCREEN_MARGIN);
            // 调整控件位置
            CGRect frame = CGRectMake(SCREEN_WIDTH - self.navigationBarRightButtonItem.frame.size.width - nav_margin,
