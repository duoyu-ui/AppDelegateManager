...
- (void)loadMoreData
{
    ...
    FYLog(NSLocalizedString(@"加载前：第[%lu]页，偏移量[%lu]，限制量[%lu]", nil), (unsigned long)weakSelf.page, (unsigned long)weakSelf.offset, (unsigned long)self.limit);
    ...
    if (![CFCNetworkReachabilityUtil isNetworkAvailable]) {
        ...
        // 下拉刷新控件，结束刷新状态
        [self.collectionViewRefresh.mj_header endRefreshing];
        
        // 上拉刷新控件，结束刷新状态
        [self.collectionViewRefresh.mj_footer endRefreshing];
        
        // 没有数据显示空白页面
        [self.collectionViewRefresh setMj_footer:nil];
        ...
        FYLog(NSLocalizedString(@"加载后：第[%lu]页，偏移量[%ld]，限制量[%ld]", nil), (unsigned long)weakSelf.page, (unsigned long)weakSelf.offset, (unsigned long)self.limit);
        ...
        [self loadNetworkDataThen:^(BOOL success, BOOL isCache, NSUInteger count){
            ...
            FYLog(NSLocalizedString(@"加载后：第[%lu]页，偏移量[%lu]，限制量[%lu]", nil), (unsigned long)weakSelf.page, (unsigned long)weakSelf.offset, (unsigned long)self.limit);
...
- (void)loadNetworkDataThen:(void (^)(BOOL success, BOOL isCache, NSUInteger count))then
{
    ...
    [NET_REQUEST_MANAGER requestWithAct:actInfo requestType:self.requestMethod parameters:params success:^(id responseObject) {
        ...
        if (listCount > 0) {
            isSuccess = YES;
            FYLog(NSLocalizedString(@"加载请求网络数据成功", nil));
        } else {
            isSuccess = YES;
            FYLog(NSLocalizedString(@"没有更多数据", nil));
        }
