...
{
#if DEBUG
    NSLog(NSLocalizedString(@"ZJScrollSegmentView ---- 销毁", nil));
...
- (void)setUpTitleViewsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        float labelEdgetMargin = self.segmentStyle.titleEdgetMargin;
        float lastLableMaxX = labelEdgetMargin + self.segmentStyle.titleMargin;
...
- (void)setSelectedIndex:(NSInteger)index animated:(BOOL)animated {
    NSAssert(index >= 0 && index < self.titles.count, NSLocalizedString(@"设置的下标不合法!!", nil));
...
- (UIView *)coverLayer {
    ...
    if (_coverLayer == nil) {
        ...
        if (self.segmentStyle.isCoverGradualChangeColor) {
            [coverView az_setGradientBackgroundWithColors:self.segmentStyle.coverGradualColors locations:nil startPoint:self.segmentStyle.coverGradualStartPoint endPoint:self.segmentStyle.coverGradualEndPoint];
        }
...
- (NSArray *)normalColorRGBA {
    if (!_normalColorRGBA) {
        ...
        NSAssert(normalColorRGBA, NSLocalizedString(@"设置普通状态的文字颜色时 请使用RGBA空间的颜色值", nil));
...
- (NSArray *)selectedColorRGBA {
    if (!_selectedColorRGBA) {
        ...
        NSAssert(selectedColorRGBA, NSLocalizedString(@"设置选中状态的文字颜色时 请使用RGBA空间的颜色值", nil));
