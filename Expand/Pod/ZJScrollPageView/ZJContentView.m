...
- (void)commonInit {
    ...
    if ([_delegate respondsToSelector:@selector(numberOfChildViewControllers)]) {
        ...
        NSAssert(NO, NSLocalizedString(@"必须实现的代理方法", nil));
    }
...
#if DEBUG
    NSLog(NSLocalizedString(@"ZJContentView---销毁", nil));
#endif
}
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildVc == nil) {
            ...
            if (!_currentChildVc || ![_currentChildVc conformsToProtocol:@protocol(ZJScrollPageViewChildVcDelegate)]) {
                NSAssert(NO, NSLocalizedString(@"子控制器必须遵守ZJScrollPageViewChildVcDelegate协议", nil));
            }
            // 设置当前下标
        ...
        NSAssert(NO, NSLocalizedString(@"必须设置代理和实现代理方法", nil));
    }
    // 这里建立子控制器和父控制器的关系
    if ([_currentChildVc isKindOfClass:[UINavigationController class]]) {
        NSAssert(NO, NSLocalizedString(@"不要添加UINavigationController包装后的子控制器", nil));
    }
    ...
    //    NSLog(NSLocalizedString(@"当前的index:%ld", nil), indexPath.row);
...
//    NSLog(NSLocalizedString(@"出现出现:current:---- %ld   old ----- %ld indexpathRow----%ld ", nil), _currentIndex, _oldIndex, indexPath.row);
    if (_sysVersion >= 8) {
        [self setupChildVcForCell:cell atIndexPath:indexPath];
    }
...
//    NSLog(NSLocalizedString(@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld ", nil), _currentIndex, _oldIndex, indexPath.row);
