...
-(void)initData{
    //    NSDictionary *dic1 = @{@"title":NSLocalizedString(@"发包满额奖励", nil),@"num":@"0"};
    //    [self.dataArray addObject:dic1];
    //
    //    NSDictionary *dic2 = @{@"title":NSLocalizedString(@"抢包满额奖励", nil),@"num":@"0"};
    //    [self.dataArray addObject:dic2];
    //
    //    NSDictionary *dic3 = @{@"title":NSLocalizedString(@"直推流水佣金", nil),@"num":@"0"};
    //    [self.dataArray addObject:dic3];
    //
    //    NSDictionary *dic4 = @{@"title":NSLocalizedString(@"邀请好友佣金", nil),@"num":@"0"};
    //    [self.dataArray addObject:dic4];
    //
    //    NSDictionary *dic5 = @{@"title":NSLocalizedString(@"豹子顺子奖励", nil),@"num":@"0"};
...
-(void)getReword:(UIButton *)btn{
    ...
    [view showWithText:NSLocalizedString(@"是否领取奖励？", nil) button1:NSLocalizedString(@"取消", nil) button2:NSLocalizedString(@"领取", nil) callBack:^(id object) {
...
-(void)xiangQingAction:(UIButton *)btn{
    ...
    vc.imageUrl = img;
    vc.hiddenNavBar = YES;
    vc.title = NSLocalizedString(@"活动详情", nil);
...
-(void)headAction:(UIButton *)btn{
    ...
    if(tag == 1){
        vc.rewardType = 1100;
        vc.title = NSLocalizedString(@"我的首充奖励", nil);
    }
    else{
        vc.rewardType = 1200;
        vc.title = NSLocalizedString(@"我的二充奖励", nil);
    }
    vc.hidesBottomBarWhenPushed = YES;
    [[[FunctionManager sharedInstance] currentViewController].navigationController pushViewController:vc animated:YES];
}
