...
- (void)sendPingWithData:(NSData *)data {
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didfail:sequenceNumber:error:)] ) {
            [strongDelegate simplePing:self didfail:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            ...
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didabnormal:)] ) {
                [strongDelegate simplePing:self didabnormal:packet];
            }
        }
    } else {
