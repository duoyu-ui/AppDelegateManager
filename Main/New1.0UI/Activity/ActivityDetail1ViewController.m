...
- (void)writeTitle
{
    ...
    if(type == RewardType_czjl)
        label.text = [NSString stringWithFormat:NSLocalizedString(@"已领取%@元", nil),amout];
    else if(type == RewardType_ztlsyj || type == RewardType_bzsz)
        label.text = [NSString stringWithFormat:NSLocalizedString(@"%@元", nil),amout];
    else
        label.text = [NSString stringWithFormat:NSLocalizedString(@"累计%@元", nil),amout];
