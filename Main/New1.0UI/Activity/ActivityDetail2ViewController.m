...
@interface ActivityDetail2ViewController ()
<UITableViewDelegate,
...
-(void)getDataBack:(NSDictionary *)dict{
    ...
    if(self.getButton){
        if(!isGet){
            [self.getButton setTitle:[NSString stringWithFormat:NSLocalizedString(@"奖励¥%@ 领取", nil),STR_TO_AmountFloatSTR(self.rewardDic[@"canGetMoney"])] forState:UIControlStateNormal];
            ...
            [self.getButton setTitle:[NSString stringWithFormat:NSLocalizedString(@"奖励¥%@ 已领取", nil),STR_TO_AmountFloatSTR(self.rewardDic[@"canGetMoney"])] forState:UIControlStateNormal];
...
-(void)updateImageShow{
    ...
    if(self.rewardDic){
        ...
        [btn setTitle:NSLocalizedString(@"请稍候", nil) forState:UIControlStateNormal];
...
-(void)getRewardAction{
    WEAK_OBJ(weakSelf, self);
    [[AlertViewCus createInstanceWithView:nil] showWithText:NSLocalizedString(@"是否领取奖励？", nil) button1:NSLocalizedString(@"取消", nil) button2:NSLocalizedString(@"领取", nil) callBack:^(id object) {
...
-(void)starViewAni{
    [UIView animateWithDuration:0.5 delay:3.0 usingSpringWithDamping:0.5 initialSpringVelocity:0.5 options:0 animations:^{
