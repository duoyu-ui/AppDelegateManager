...
+(NSInteger)cellHeight{
    return SCREEN_WIDTH * (360 / 1040.0) ;
}
...
-(void)initView{
    UIView *containView = [[UIView alloc] initWithFrame:CGRectMake(12, 3, SCREEN_WIDTH - 24, [ActivityDetailCell cellHeight] - 6)];
    ...
    titleBgView.backgroundColor = COLOR_X(249, 217, 41);
    [containView addSubview:titleBgView];
    ...
    [titleBgView addSubview:progressBg];
    [progressBg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(titleBgView.mas_right).offset(-15);
    ...
    [titleBgView addSubview:self.percentLabel];
    [self.percentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(titleBgView.mas_height);
    ...
    [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(titleBgView).offset(12);
        make.top.bottom.equalTo(titleBgView);
    ...
    UIView *view1 = [self viewForIcon:@"activityIcon1" title:NSLocalizedString(@"18.88奖金", nil)];
    ...
    UIView *view2 = [self viewForIcon:@"activityIcon2" title:NSLocalizedString(@"1次抽奖", nil)];
...
-(void)setTitle:(NSString *)title percentString:(NSString *)percentStr percent:(float)percent reward:(NSString *)reward lottery:(NSString *)lottery{
    ...
    self.rewardLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@奖金", nil),reward];
    self.lotteryLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@次抽奖", nil),lottery];
