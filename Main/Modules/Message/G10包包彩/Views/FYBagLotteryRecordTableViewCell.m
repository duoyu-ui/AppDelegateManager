//
//  FYBagLotteryRecordTableViewCell.m
//  ProjectCSHB
//
...
@interface FYBagLotteryRecordTableViewCell ()
/**
 * 根容器组件
 */
...
@implementation FYBagLotteryRecordTableViewCell
...
#pragma mark - 设置数据模型
- (void)setModel:(FYBagLotteryRecordModel *)model
{
    if (![model isKindOfClass:[FYBagLotteryRecordModel class]]) {
        return;
    }
    ...
    // 开奖
    [self.winnerLabel setText:self.model.winNumbers];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectRowAtBagLotteryRecordModel:indexPath:)]) {
        [self.delegate didSelectRowAtBagLotteryRecordModel:self.model indexPath:self.indexPath];
    }
}
