//
//  FYJSSLGameRecordTableViewCell.m
//  ProjectCSHB
//
...
@interface FYJSSLGameRecordTableViewCell ()
/**
 * 根容器组件
 */
...
@implementation FYJSSLGameRecordTableViewCell
...
#pragma mark - 设置数据模型
- (void)setModel:(FYJSSLGameRecordModel *)model
{
    if (![model isKindOfClass:[FYJSSLGameRecordModel class]]) {
        return;
    }
    ...
    // 开奖
    [self.winnerLabel setText:self.model.result];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectRowAtJSSLGameRecordModel:indexPath:)]) {
        [self.delegate didSelectRowAtJSSLGameRecordModel:self.model indexPath:self.indexPath];
    }
}
