//
//  FYBaiRenNNRecordTableViewCell.m
//  ProjectCSHB
//
...
@interface FYBaiRenNNRecordTableViewCell ()
/**
 * 根容器组件
 */
...
@implementation FYBaiRenNNRecordTableViewCell
...
- (void)createViewAtuoLayout
{
    ...
    NSArray<NSNumber *> *percents = @[ @(0.20f), @(0.40f), @(0.60f), @(0.93f), @(1.0f) ];
...
#pragma mark - 设置数据模型
- (void)setModel:(FYBaiRenNNRecordModel *)model
{
    if (![model isKindOfClass:[FYBaiRenNNRecordModel class]]) {
        return;
    }
    ...
    // 期数
    [self.issueLabel setText:[NSString stringWithFormat:@"%@", self.model.period]];
    
    // 开奖
    if (VALIDATE_STRING_EMPTY(self.model.winStr)) {
        [self.winnerLabel setText:STR_APP_TEXT_PLACEHOLDER];
    } else {
        NSMutableArray<NSString *> *winTextArr = [NSMutableArray array];
        NSArray<NSString *> *splitArr = [self.model.winStr componentsSeparatedByString:@","];
        for (NSString *cardsInfo in splitArr) {
            NSArray<NSString *> *cardsArr = [cardsInfo componentsSeparatedByString:@"|"];
            if (cardsArr.count > 0) {
                [winTextArr addObj:cardsArr.firstObject];
            }
        }
        NSString *winnerString = [winTextArr componentsJoinedByString:@""];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectRowAtBaiRenNNRecordModel:indexPath:)]) {
        [self.delegate didSelectRowAtBaiRenNNRecordModel:self.model indexPath:self.indexPath];
    }
}
