...
NSString * const kPersonalSignatureHeadCellId = @"kPersonalSignatureHeadCellId";


@interface PersonalSignatureHeadCell ()
...
@implementation PersonalSignatureHeadCell
...
{
    PersonalSignatureHeadCell *cell = (PersonalSignatureHeadCell *)[tableView dequeueReusableCellWithIdentifier :kPersonalSignatureHeadCellId];
    if (!cell) {
        cell = [[PersonalSignatureHeadCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPersonalSignatureHeadCellId];
    }
    return cell;
}
...
- (void)setContacts:(FYContacts *)contacts personalModel:(PersonalSignatureModel *)personalModel
{
    ...
    // 新代码4.0
    {
        ...
        // 用户账号
        if (self.isShowID || contacts.isFriend) {
            ...
            self.userIdLabel.text = NSLocalizedString(@"手机: ***********", nil);
        }
    }
