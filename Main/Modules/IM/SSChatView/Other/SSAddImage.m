...
-(void)getImagePickerWithAlertController:(UIViewController *)controller modelType:(SSImagePickerModelType)modelType pickerBlock:(SSAddImagePicekerBlock)pickerBlock{
    ...
    NSArray *alerts = @[@{SSPickerWayGallery:NSLocalizedString(@"相册", nil)},
                        @{SSPickerWayCamer:NSLocalizedString(@"拍摄", nil)}];
...
-(void)addImagePickerFromCamer:(SSImagePickerModelType)modelType{
    ...
    if(![self isCameraAvailable]){
        NSLog(NSLocalizedString(@"摄像头不可用", nil));
        return;
    }
...
- (void)addImagePickerFromIpc:(SSImagePickerModelType)modelType{
    ...
    if(![self isPhotoLibraryAvailable]){
        NSLog(NSLocalizedString(@"相册不可用", nil));
        return;
    }
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    if (error) {
        NSLog(NSLocalizedString(@"保存视频过程中发生错误，错误信息:%@", nil),error.localizedDescription);
    }else{
        NSLog(NSLocalizedString(@"视频保存成功.", nil));
        if(_pickerBlock){
