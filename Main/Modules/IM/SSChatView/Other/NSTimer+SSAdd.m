...
+ (NSString *)getChatTimeStr:(long)timestamp{
    ...
    if (currentYear == msgYear && currentMonth == msgMonth && currentDay == msgDay) {
        //今天
        dateFmt.dateFormat = [NSString stringWithFormat:@"%@ HH:mm",NSLocalizedString(@"今天", nil)];
    }else if (currentYear == msgYear && currentMonth == msgMonth && currentDay-1 == msgDay ){
        //昨天
        dateFmt.dateFormat = [NSString stringWithFormat:@"%@ HH:mm",NSLocalizedString(@"昨天", nil)];
    }else{
        //昨天以前
...
+(NSTimeInterval)CompareTwoTime:(NSTimeInterval)time1 time2:(NSTimeInterval)time2{
    NSTimeInterval balance = fabs(time2- time1);
