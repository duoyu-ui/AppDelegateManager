...
#import "SSChatBagLotteryResultsCell.h"
#import "FYBagLotteryResultsCell.h"
static NSString *const FYBagLotteryResultsCellID = @"FYBagLotteryResultsCellID";
@interface SSChatBagLotteryResultsCell ()<UITableViewDelegate,UITableViewDataSource>
...
@end
@implementation SSChatBagLotteryResultsCell
- (void)setModel:(FYMessagelLayoutModel *)model{
    ...
    SSChatBagLotteryResultsModel *winModel = [SSChatBagLotteryResultsModel mj_objectWithKeyValues:dict];
    ...
    self.resultLab.text = winModel.data.result;
    self.open = [SSChatBagLotteryResultsOpen mj_objectArrayWithKeyValuesArray:winModel.data.open];
...
- (void)initChatCellUI{
    ...
    [self.bubbleBackView addSubview:self.resultLab];
    ...
    [self.lineView1 mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(kLineHeight);
        make.bottom.equalTo(self.lineView.mas_top).offset(kLineSpacing * 2 + 20 * 2);
    }];
    ...
    [self.resultLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.titleLab);
        make.bottom.mas_equalTo(self.lineView1.mas_top).offset(-kLineSpacing);
    }];
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[FYBagLotteryResultsCell class] forCellReuseIdentifier:FYBagLotteryResultsCellID];
    }
    return _tableView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    FYBagLotteryResultsCell *cell = [tableView dequeueReusableCellWithIdentifier:FYBagLotteryResultsCellID];
    if (self.open.count > indexPath.row) {
        cell.list = self.open[indexPath.row];
    }
    return cell;
}
/**cell个数*/
...
- (UILabel *)resultLab{
    if (!_resultLab) {
        ...
        _resultLab.textColor = HexColor(@"#1A1A1A");
        _resultLab.font = [UIFont systemFontOfSize:kCellFont];
    }
    return _resultLab;
}
- (UILabel *)endLab{
...
@implementation SSChatBagLotteryResultsOpen

@end
@implementation SSChatBagLotteryResultsData

@end
@implementation SSChatBagLotteryResultsModel
