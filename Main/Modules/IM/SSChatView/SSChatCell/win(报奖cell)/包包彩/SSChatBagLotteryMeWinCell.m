...
#import "SSChatBagLotteryMeWinCell.h"
#import "FYBegLotteryHistoryModel.h"
@interface SSChatBagLotteryMeWinCell()
...
@property (nonatomic , strong) NSMutableArray <UILabel*> *labNumArr;
@end
@implementation SSChatBagLotteryMeWinCell
...
- (void)setModel:(FYMessagelLayoutModel *)model{
    ...
    NSArray <NSString*>*msg = [NSString mj_objectArrayWithKeyValuesArray:dict[@"msg"]];
    
    self.titleLab.text = [NSString stringWithFormat:@"%@❤%@",msg.firstObject,msg[1]];
    ...
    NSArray <NSString *>*object = [msg.lastObject componentsSeparatedByString:@","];
    ...
    [self.labNumArr enumerateObjectsUsingBlock:^(UILabel *lab, NSUInteger idx, BOOL * _Nonnull stop) {
        lab.text = object[idx];
        lab.backgroundColor = [FYBegLotteryTool setLabBackgroundColor:object[idx]];
    }];
}
- (void)initChatCellUI{
    ...
    [self.lineView1 mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(kLineHeight);
        make.bottom.equalTo(self.bubbleBackView.mas_bottom).offset(-(kLineSpacing * 2 + 25));
    }];
    for (int i = 0; i < 4; i++) {
        UILabel *lab = [self setNumLabCent];
    ...
    for (int i = 0; i < 6; i++) {
           ...
           [lab mas_makeConstraints:^(MASConstraintMaker *make) {
               ...
               make.left.equalTo(self.lineView.mas_left).offset(5 + i * 29);
               make.height.mas_equalTo(25);
               make.bottom.equalTo(self.bubbleBackView.mas_bottom).offset(-kLineSpacing);
           }];
           [self.labNumArr addObj:lab];
       }
...
- (NSMutableArray<UILabel *> *)labNumArr{
    if (!_labNumArr) {
        _labNumArr = [NSMutableArray array];
    }
    return _labNumArr;
}
@end
