...
#import "SSChatBagLotteryBetCell.h"
...
- (void)setModel:(FYMessagelLayoutModel *)model{
    ...
    if (model.message.messageType == FYMessageTypeBestNiuNiuBett) {//百人牛牛
...
- (void)copyItem{
    if(self.delegate && [self.delegate respondsToSelector:@selector(didChatBagLotteryBetCell:row:)]){
           [self.delegate didChatBagLotteryBetCell:self.model.message row:3];
       }
       [[GMenuController sharedMenuController] setMenuVisible:NO];
}
- (void)toVoteForItem{
