...
-(void)SSChatKeyBordSymbolCellClick:(NSObject *)emojiText{
    if ([emojiText isEqual:DeleteButtonId]) {
        if (_delegate && [_delegate respondsToSelector:@selector(SSChatKeyBordSymbolViewDidDeleteBackward:)]) {
            [_delegate SSChatKeyBordSymbolViewDidDeleteBackward:YES];
        }
    }else {
...
-(void)SSChatKeyBordButtonPressed:(NSInteger)index{
    if(_delegate && [_delegate respondsToSelector:@selector(fyChatFunctionBoardClickedItemWithTag:type:)]){
        [_delegate fyChatFunctionBoardClickedItemWithTag:index type:_type];
    }
}
