...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _sendButton .frame = CGRectMake(FYSCREEN_Width-80, 0, 80, self.height);
        _sendButton.tag = 200;
        ...
        _emojiFooterScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, FYSCREEN_Width - _sendButton.width, self.height)];
        _emojiFooterScrollView.showsHorizontalScrollIndicator = NO;
        _emojiFooterScrollView.showsVerticalScrollIndicator = NO;
        _emojiFooterScrollView.contentSize = CGSizeMake(FYSCREEN_Width - _sendButton.width+1,  self.height);
        [self addSubview:_emojiFooterScrollView];
        
        
        _mButton1 = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, FYSCREEN_Width / 6, 40)];
        _mButton1.tag = 500;
...
//        _mButton2 = [[UIButton alloc] initWithFrame:CGRectMake(FYSCREEN_Width / 6, 0, FYSCREEN_Width / 6, 40)];
//        _mButton2.tag = 501;
...
-(void)buttonPressed:(UIButton *)sender{
    NSLog(NSLocalizedString(@"点击了发送", nil));
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _footer = [[SSChatKeyBordSymbolFooter alloc] initWithFrame:CGRectMake(0, self.height-SSChatKeyBordSymbolFooterH, FYSCREEN_Width, SSChatKeyBordSymbolFooterH)];
        ...
        self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, FYSCREEN_Width, self.height-_footer.height-20) collectionViewLayout:self.layout];
        ...
        _pageControl.centerX = FYSCREEN_Width*0.5;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scrollView == self.collectionView) {
        if (scrollView.contentOffset.x >= FYSCREEN_Width * _numberPage1){
...
//        [self setSymbolValue1];
        ...
        [self.collectionView setContentOffset:CGPointMake(FYSCREEN_Width * _numberPage1, self.collectionView.contentOffset.y) animated:YES];
        [self setSymbolValue2];
...
-(void)setSymbolValue1{
    ...
    self.pageControl.currentPage = (self.collectionView.contentOffset.x / FYSCREEN_Width);
...
-(void)setSymbolValue2{
    ...
    self.pageControl.currentPage = ((self.collectionView.contentOffset.x - _numberPage1*FYSCREEN_Width) / FYSCREEN_Width);
