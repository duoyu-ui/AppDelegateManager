...
extension String {
    ...
    // MARK: 群名称正则表达式
    func validateGroupName() -> Bool {
        let phoneRegex = try? NSRegularExpression(pattern: "^[\\u4e00-\\u9fa5_a-zA-Z0-9]+$", options: NSRegularExpression.Options.caseInsensitive)
...
extension String {
    /// 是否包含空/空格/null字符
    var isBlank: Bool {
        return allSatisfy({ $0.isWhitespace }) || self.isContainsNull
    }
}
...
extension Optional where Wrapped == String {
    var isContainsNull: Bool {
        return self?.isContainsNull ?? true
    }
}
