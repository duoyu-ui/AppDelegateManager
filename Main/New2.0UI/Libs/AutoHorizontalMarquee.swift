...
//
//  Created by Jetter on 2018/8/13.
//  Copyright © 2018年 Jetter. All rights reserved.
//
...
extension AutoHorizontalMarquee {
    ...
    func reloadData(_ text: String) {
        DispatchQueue.main.async {
            self.autoLabel.text = text
            self.autoLabel.sizeToFit()
            // setup center
            self.autoLabel.centerY = self.height * 0.5
        }
    }
...
extension AutoHorizontalMarquee {
    ...
    @objc func tapAction() {
        ...
        delegate?.autoDidSelectIndex(self, index: autoIndex)
    }
}
