//
//  FYGamesMode1HBGroupController.m
//  ProjectCSHB
//
...
static NSString * const CELL_IDENTIFIER_GAMES_MODE1_TABLEVIEW_CLASS_CONTENT = @"FYGamesMode1HBGroupControllerContentCellIdentifier";

@interface FYGamesMode1GroupCustomTableView : UITableView
@end
@implementation FYGamesMode1GroupCustomTableView
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    return [gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && [otherGestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]];
}
@end

@interface FYGamesMode1HBGroupController () <ZJScrollPageViewDelegate, FYGamesMode1HBGroupSubControllerDelegate, UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource>
...
@property (strong, nonatomic) FYGamesMode1GroupCustomTableView *tableView;
@property (strong, nonatomic) FYGamesMode1HBGroupSectionHeader *tableSectionHeaderView;
...
@implementation FYGamesMode1HBGroupController
    ...
    [self loadRequestMenuData:self.groupDataSource then:^(BOOL success, NSMutableArray<FYGamesMode1ClassModel *> *itemTabTypeModels) {
        // 菜单列表
        {
            ...
            [itemTabTypeModels enumerateObjectsUsingBlock:^(FYGamesMode1ClassModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [tabTitleCodes addObj:[NSString stringWithFormat:@"%@", obj.type.stringValue]];
                if (obj.uuid.integerValue == weakSelf.selectedGroupModel.uuid.integerValue) {
                    tabTitleCodeOfSelected = obj.type.stringValue;
                }
...
- (void)loadRequestMenuData:(NSMutableArray<FYGamesMode1ClassModel *> *)dataSource then:(void (^)(BOOL success, NSMutableArray<FYGamesMode1ClassModel *> *itemTabTypeModels))then
{
    if (!dataSource || dataSource.count <= 0) {
       !then ?: then(NO,nil);
        ...
        for (FYGamesMode1ClassModel *obj in dataSource) {
            if (obj.openFlag.boolValue && !obj.maintainFlag.boolValue) {
                [itemTabTypeModels addObj:obj];
            }
        }
...
- (UIViewController<ZJScrollPageViewChildVcDelegate> *)childViewController:(UIViewController<ZJScrollPageViewChildVcDelegate> *)reuseViewController forIndex:(NSInteger)index
{
    ...
    if (!childViewController) {
        ...
        childViewController = [[FYGamesMode1HBGroupSubController alloc] initWithTabTitleCode:tabTitleCode tabTypeModel:tabTypeModel];
    ...
    FYGamesMode1HBGroupSubController<FYGamesMode1HBGroupControllerProtocol> *viewController = (FYGamesMode1HBGroupSubController<FYGamesMode1HBGroupControllerProtocol> *)childViewController;
...
{
    UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:CELL_IDENTIFIER_GAMES_MODE1_TABLEVIEW_CLASS_CONTENT];
    if (cell == nil) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CELL_IDENTIFIER_GAMES_MODE1_TABLEVIEW_CLASS_CONTENT];
    }
...
{
    return NSLocalizedString(@"红包游戏", nil);
}
...
- (FYGamesMode1GroupCustomTableView *)tableView
{
    if (!_tableView) {
        CGRect frame = CGRectMake(0.0f, 0.0f, self.view.bounds.size.width, self.view.bounds.size.height);
        FYGamesMode1GroupCustomTableView *tableView = [[FYGamesMode1GroupCustomTableView alloc] initWithFrame:frame style:UITableViewStylePlain];
...
- (FYGamesMode1HBGroupSectionHeader *)tableSectionHeaderView
{
    if (!_tableSectionHeaderView) {
        _tableSectionHeaderView = [[FYGamesMode1HBGroupSectionHeader alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.tableSectionHeaderHeight) headerViewHeight:self.tableSectionHeaderHeight parentViewController:self segmentView:self.segmentView];
    }
...
- (void)doRefreshForAgentReportTable
{
    ...
    dispatch_main_async_safe(^{
        if (weakSelf.delegate_subclass && [weakSelf.delegate_subclass respondsToSelector:@selector(doRefreshForHBGroupSubContentTableScrollToTopAnimated:)]) {
            [weakSelf.delegate_subclass doRefreshForHBGroupSubContentTableScrollToTopAnimated:NO];
        }
...
{
    if (self.delegate_subclass && [self.delegate_subclass respondsToSelector:@selector(doRefreshForHBGroupSubController:)]) {
        ...
        [self.delegate_subclass doRefreshForHBGroupSubController:tabTitleCode];
    }
}
